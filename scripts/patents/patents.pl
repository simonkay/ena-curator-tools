...
# This script converts old-format patent data files into a file containing
# embl-formatted entries.
...
grab_and_save_all_req_accessions();
...
while ( <IN> ) {
  ...
  if ( $line =~ /\/End of file\// ) {
    ...
    # starts a new sequence, hope this is reliable...
  ...
  ### priority block: there can be more thane one
  ...
  elsif ( $line =~ /^\((xiv|XIV)\)\s*ABSTRACT\s*:\s*(.*)$/ ) {
    $doc->abstract ( $2 );
    $flag = 'ABSTRACT';
  }
    ...
    if ( uc($1) eq 'ARTIFICIAL SEQUENCE' ) {
      $seq->qualifiers ( $seq->get_cur_feat(), "/organism=\"synthetic construct\"" );
    }
    elsif ($1 =~ /(.*)\sstrain\s(.*)/) {
      $seq->qualifiers ( $seq->get_cur_feat(), "/organism=\"$1\"");
      $seq->qualifiers ( $seq->get_cur_feat(), "/strain=\"$2\"" );	
    }
    else {     	
      $seq->qualifiers ( $seq->get_cur_feat(), "/organism=\"$1\"" );
    }
  ...
  ## hopefully temporarily as part of the old format....
  elsif ( $line =~ /(\d*)\.(Prot) Length:\s*(\d+)/ || 
	  $line =~ /(\d*)\.(Dna) Length:\s*(\d+)/ || 
	  $line =~ /(\d*)\.(Rna) Length:\s*(\d+)/ ) {
    $flag = 'SEQ';
    ...
    $seq->seqlen ( $3 );
    my ( $xftype ) = uc($2) eq 'PROT' ? 'PRT' : uc ($2);

    my $no = $1;
    $no =~ s/^0+//; # delete leading zeros
    $seq->seqno ( $no );

    if ( $xftype ne $seq->ftype() && ( $xftype eq 'PRT' || $seq->ftype() eq 'PRT' )) {
      # dna, rna, prt (prot) should be consistent
      # ( suppressed warning for types other than 'PRT' on Tamaras request )
      print_log ( \*LOG, "used different ftypes $xftype and ".$seq->ftype(), 
		 $doc, $seq );
    }
  }
...
#    $flag = 'FEATURE';
  ...
  elsif ( $line =~ /^\<211\>/ ) {  # length; ignore
    ;
  }

  elsif ( $line =~ /^\<221\>\s(.*)\s*$/ ) {  # key
    $seq->incr_cur_feat();
    ## test comes later ( as seqno not known yet to give proper error message )
    $seq->features ( $seq->get_cur_feat(), $1 );
  }
  elsif ( $line =~ /^\<222\>\s(.*)\s*$/ ) { # location
      my $loc = $1;    
    $loc =~ s/\s+//g;
    $loc =~ s/\(//g;
    $loc =~ s/\)//g;   
    $loc = lc($loc);
    $seq->locations ( $seq->get_cur_feat(), $loc );       
  }
  elsif ( $line =~ /^\<223\>\s(.*)\s*$/ ) { # feature information
    ### parse for note etc.
    my $feat = $1;

    if ( $feat =~ /^\/?note\s*\=?\:?\s*(.*)\s*$/i ) {
      $feat = delete_quotes ( $1 );
      #$qualifiers[$cur_feat] .= "/note=\"$feat\"%%";      
      $seq->qualifiers ( $seq->get_cur_feat(), "/note=\"$feat\"" );

    }
    elsif ( $feat =~ /^\/?strain\s*\=?\:?\s*(.*)\s*$/i ) {
      $feat = delete_quotes ( $1 );
      $seq->qualifiers ( $seq->get_cur_feat(), "/strain=\"$feat\"" );
    }
    elsif ( $feat =~ /^\/?tissue(\s|\_)type\s*(\=|\:)?\s*(.*)\s*$/i ) {
      $feat = delete_quotes ( $3 );
      $seq->qualifiers ( $seq->get_cur_feat(), "/tissue_type=\"$feat\"" );
    }
    else {
      $feat = delete_quotes ( $feat );
      $seq->qualifiers ( $seq->get_cur_feat(), "/note=\"$feat\"" );
    }
    ...
    # <223> /note= the name of the polynucleotide is PLT121
    # <223> /note='Linker containing a PacI site'
    # <223> /Note='linker sequence'
    # <223> /note='Product=' SsgA' / gene= 'ssgA', strain ATTC
    # <223> /Note = 'primer conIgG1-4'
    # <223> Description: Peptide suitable to inhibit HPV E7 proteins
    # <223> Description of Artificial Sequence: Basic
    # <223> SEQUENCE DESCRIPTION artificielle:mimotope
    # <223> Beschreibung der k nstlichen Sequenz: Varianten
    # <223> Beschreibung der kunstlichen Sequenz:Oligonukleotid
    # <223> Description of Unknown Organism: YEAST
    # <223> splicing boundary: 1 - 10: intron ; 11 - 20: exon
    # <223> 70-99 : CDR-L1 145-165 : CDR-L2 262-285 
    # <223> Intron 1: start at nucleotide 167
    # <223> N=unsure/label=Y2
    # <223> replace (372, 'g')
    # <223> as above
    ...
    elsif ( $flag eq 'ABSTRACT' ) {
      $doc->abstract ( $doc->abstract()." $line" );
    }
    elsif ( $flag eq 'SEQ' ) {
      $seq->seq ( $seq->seq().$line );
    }
    elsif ( $flag eq 'FEATURE' ) {
      # add only if there is already a qualifier ( otherwise its junk )
      if ( $seq->qualifiers ( $seq->get_cur_feat() ) ne '' ) {
	$line = delete_quotes ( $line );
	$seq->qualifiers ( $seq->get_cur_feat(), "++ ".$line."\"" );  
      }
    }
    else {
...
sub grab_and_save_all_req_accessions {

    my $total_num_accessions_req = `grep -c "^<212>" $fname`;
    $total_num_accessions_req =~ s/\s*$//g;
...
sub format_date1 {
    ...
    my ($new_date_format, $ch_month, $month, $year, $day);
	...
	if ($month == 1) { $ch_month = 'JAN'; }
	elsif ($month == 2 ) { $ch_month = 'FEB'; }
	elsif ($month == 3 ) { $ch_month = 'MAR'; }
	elsif ($month == 4 ) { $ch_month = 'APR'; }
	elsif ($month == 5 ) { $ch_month = 'MAY'; }
	elsif ($month == 6 ) { $ch_month = 'JUN'; }
	elsif ($month == 7 ) { $ch_month = 'JUL'; }
	elsif ($month == 8 ) { $ch_month = 'AUG'; }
	elsif ($month == 9 ) { $ch_month = 'SEP'; }
	elsif ($month == 10) { $ch_month = 'OCT'; }
	elsif ($month == 11) { $ch_month = 'NOV'; }
	elsif ($month == 12) { $ch_month = 'DEC'; }

	$new_date_format = "$day-$ch_month-$year";
    }
    else {
...
sub test_entry {
  ...
  if ( ! $doc->is_tested() ) {
    ...
    $doc->docdate( format_date1($doc->docdate(), \*$FH, $doc) );
    $doc->appdate( format_date1($doc->appdate(), \*$FH, $doc) );

    for ( my $i = 0; $i < $doc->nof ('prioappdate'); $i++ ) {
      #$doc->set_prioappdate ( $i, format_date ( $db, $doc->get_prioappdate($i), \*$FH, $doc ));
      $doc->set_prioappdate( $i, format_date1($doc->get_prioappdate($i), \*$FH, $doc) );
    }
...
#	    "select tax_id from ntx_synonym where upper_name_txt = '$org_name'" );
    ...
    # no organism given
    $seq->qualifiers ( 0, "/organism=\"synthetic construct\"" );
  }
}
...
sub get_accession {
  ...
  #my $call = "select d.primaryacc#
  #              from dbentry d, patent p, patent_bioseq pb
  #             where p.docoffice = '".$doc->docoffice()."'
  #               and p.docnum = '".$doc->docnumber()."'
  #               and p.doctype = '".$doc->doctype()."'
  #               and p.pubid = pb.pubid
  #               and pb.orderin = ".$seq->seqno()."
  #               and pb.seqid = d.bioseqid ";
...
sub print_entry {
  ...
  if (($doc->invname() eq "") && ($doc->appname() ne "")) {
      ...
      print $FH  "RL   Patent number ".$doc->docoffice().$doc->docnumber()."-".$doc->doctype().
