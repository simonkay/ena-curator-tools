...
my $lockfile = '/ebi/production/seqdb/embl/tools/curators/scripts/ftp_release/report_digest.LOCK';
...
sub main {
    ...
    my @type_codes = ( 'normal',
		       'con'
		     );

    foreach my $type (@type_codes) {

	my FtpRelease::ReleaseConfig $config = FtpRelease::ReleaseConfig->get_config( $type, 1 );
...
sub write_report_digest {
  my FtpRelease::ReleaseConfig $config = shift;
