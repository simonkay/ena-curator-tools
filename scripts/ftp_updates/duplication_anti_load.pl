...
# 24-Jan-2006 F. Nardone            CDS added to the excluded list
#===============================================================================
...
my ($output_file)       = $ARGV[1] ? $ARGV[1] : $input_file . "_out";
my ($qual_continuation) = 0;
my ($key_continuation)  = 0;
my ($keys, $data, $qualifiers);
my ($key, $qualifier, $number);
my ($index, $i, $j);
my (%excluded) = ( 'CDS', 1, "source", 1, "conflict", 1, "misc_difference", 1, "old_sequence", 1, "unsure", 1, "variation", 1);
