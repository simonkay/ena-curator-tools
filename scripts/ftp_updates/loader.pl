...
sub main {
    ...
    my ( $dblogin, $collaborator, $type, $verb ) = get_args();
    ...
    my FtpUpdates::Config $config = FtpUpdates::Config->get_config( $collaborator, $type, $verb );
    ...
    if ($config->{entry_type_name} =~ /WGS/i) {
  ...
  foreach my $file_name ( @files_gzip, @files_to_just_move ) {
    ...
    if ( -s( "$config->{ftp_local_dir}/$file_name" ) > 0 ) {
      $command = "mv $config->{ftp_local_dir}/$file_name $config->{uncompressed_dir}";
        ...
        if(-f $config->{ftp_local_dir}/$file_name){
...
sub merge_duplicates {
  ...
  print STDERR "Done.\n" if ( $config->{VERBOSE} );
  print STDERR  scalar(localtime(time + 0)), "\n";
  sleep(60);
...
sub load_wgs {
  ...
  # Do only 5 at a time to avoid Lsf deadlocks when we
...
sub group_reports {
  ...
  if (@files) {
    ...
    unless( -f($tar_fname) ) {
      ...
      #my $cmd = "tar --warning=no-file-changed -czf $tar_fname $fname || [[ $? -eq 1 ]]" if (-e($config->{logs_dir}/$fname));# create archive
      #my $cmd = "tar  --warning=no-file-changed --ignore-failed-read --warning=no-file-changed --warning=no-file-removed  -cvWf $tar_fname $fname"; # create archive
      my $cmd = "tar  -cvf $tar_fname $fname || [[ \$\? -eq 1 ]]";
      print "Running: $cmd\n";
      my_system( $cmd );
      my_unlink( $fname );
    }
    ...
    foreach my $fname ( @files ){
      ...
      #my $cmd = "tar --warning=no-file-changed --ignore-failed-read -rvf $tar_fname $fname"; # Add to archive
      #my $cmd = "tar --warning=no-file-changed --ignore-failed-read -rf $tar_fname $fname  || [[ $? -eq 1 ]]" if (-e($config->{logs_dir}/$fname));# add to archive
      print "Running: $cmd\n";
      my_system( $cmd );
      my_unlink( $fname );
    }
...
sub get_args {
  ...
  my ( $dblogin, $collaborator, $type, $verb ) = @ARGV;
...
#                     -wgs    => TYPE_WGS   }->{$type};
  ...
  return ( $dblogin, $collaborator, $type_code, $verb );
}
