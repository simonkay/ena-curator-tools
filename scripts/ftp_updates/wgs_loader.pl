...
sub main {
  ...
  my ( $dblogin, $collaborator, $set_prefix, $verbose ) = get_args();

  my FtpUpdates::Config $config = FtpUpdates::Config->get_config( $collaborator,
                                                                  'TYPE_WGS',
                                                                  $verbose );
  ...
  if ( $fnames_ar ) {
    ...
    my $full_set_id = get_set_id_version( $config, $fnames_ar );
    my ($description, $error) = get_description( $config, $fnames_ar );
    my $added = add_new_prefix( $config, $dblogin, $set_prefix, $description );
    my $body = scalar( localtime( time() ) ) ."\n\n" .
	"WGS prefix '$set_prefix', 'WGS - $description' ($config->{collaborator}) added to the database.\n";
    if (defined($error)) {
	$body .= $error;
    }
    if ( $added ) {
	send_mail( { to      => $config->{notification_addresses},
		     subject => "WGS prefix '$set_prefix' ($config->{collaborator}) added to the database.",
		     body    => $body
		 } );
    }
    ...
    unless( @$failed_jobs_id || $loading_status->{failed} > 0 ) {
     ...
     $dblogin =~ s/^\/?\@?//;
      ...
      add_prefix_to_distribution_table( $dbh, $full_set_id );
...
sub get_file_names {
  ...
  my $match_regexp = $config->{wgs_fname_begin} . $set_prefix;

  my $fnames  = [grep( m/$match_regexp/, @$all_fnames )];
#  if($config->{collaborator} eq DDBJ){
...
sub notify_wgs_loading {
  ...
  my $msg = "WGS '$full_set_id', description '$description':\n".
            "\n".
   sprintf( "stored    = %8d\n", $status->{stored} ).
   sprintf( "failed    = %8d\n", $status->{failed} ).
  ...
  if ( (scalar( @$failed_files ) > 0) || 
    ...
    $subject = "ERROR: WGS $full_set_id loading, $description";

  } else {

    $msg .= "The set will be distributed.\n";
    $subject = "WGS $full_set_id loaded, $description";
  }
...
sub get_set_id_version {
  ...
  if ( !defined($full_id) ) {
    die( "Can't find a valid WGS accession number in '$config->{fixed_dir}/$fnames_ar->[0]'\n" );
  }
 ...
 $dblogin =~ s/^\/?\@?//;
  ...
  if ( !defined( $res->[0] ) ) {# the prefix is not already in the cv_table
    ...
    my $add_sql = "INSERT INTO cv_database_prefix
                   ...
                   VALUES ('$config->{db_code}', '$prefix', 'WGS - $description')";
...
sub add_prefix_to_distribution_table {
  ...
  my $sql = "INSERT INTO distribution_wgs
                    (wgs_set,       distributed)
             VALUES ('$set_prefix', 'N')";
...
sub get_args {
  ...
  my ( $dblogin, $collaborator, $set_prefix, $verbose ) = @ARGV;
  ...
  return( $dblogin, $collaborator, $set_prefix, $verbose );
}
