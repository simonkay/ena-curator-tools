...
#
#  $Header: /ebi/cvs/seqdb/seqdb/tools/curators/scripts/ckprots.pl,v 1.16 2009/10/08 09:43:03 gemmah Exp $
#
#  (C) EBI 2000
#
#  MODULE DESCRIPTION:
#
#  Parses the log file of the command 'putff -parse_only' (which makes a
...
while ( <IN> ) 
{
   ...
   if ( $parse_only &&
	( $line =~ /\#{10} NEXT ENTRY \#{10}/ 
	  ...
	  $line =~ /^total cpu/ ) ) {
       ...
       $CDSmessage = "";
       last if $line =~ /^total cpu/; # end of entries
   }
   ...
   if ( $line =~ /\-{30}/ ) 
   {
	 ...
	 {
	    my $message = '';
	    $error   ne '' && compose_message ( \$message, "E", $error );
	    $warning ne '' && compose_message ( \$message, "W", $warning );
	    if ($pseudo eq ''){
		$CDSmessage .= $message;
	    }
	    else {
	 ...
	 chomp ( $line = $_ );
	 # CDS-location-line
	 last if $line =~ /^(ERROR )?CDS \S*/;

	 # end of CDS TRANSLATIONS block:
	 # 1. failure: 'proteintranslation failed' message
	 last if $line =~ /^ERROR proteintranslation failed for entry/;
#	 last if $line =~ /\-{30}/; # just in case
	 # 2. success: empty line or 'total elapsed time'
	 last if $line eq ""; # pointless?
	 last if $line =~ /^total elapsed time/;
	 ...
	 $filePositionMarker = tell(IN);
      ...
      if ( $line =~ /^CDS (\S*)/ || $line =~ /^ERROR CDS (\S*)/ )
      {
	 $CDSmessage .= sprintf "\n%-10s %-15s  ", $first_cds ? $accno : "", substr ( $1, 0, 15 );
	 $first_cds = 0 if $first_cds;
      }

      if ( $line =~ /^CDS \S*\s+\/(pseudo|non_functional)/){
	  $pseudo = $1;
      }
  }
   ...
   {
      $first_aa = substr ( $line, 0, 1 ) if ( $first_aa eq ' ' );
      $line =~ /([\*\w])\s*$/;
      $last_aa = $1; 
   }
...
sub compose_message 
{
   my ( $r_message, $type, $text ) = @_;

   add_message ( $r_message, $text, "contains stop codon", "contains stop" );
   add_message ( $r_message, $text, "missing stop codon", "missing stop" );
   add_message ( $r_message, $text, "invalid start codon", "invalid start" );
   add_message ( $r_message, $text, "must be 5\' partial" );
   add_message ( $r_message, $text, "must be 5\' or/and 3\' partial" );
   add_message ( $r_message, $text, "requires \/codon\_start", "add /codon_start" );
   add_message ( $r_message, $text, "resubmit entry with qualifier \/codon\_start\=(\\d)", 
			                 "try /codon_start=" );
   add_message ( $r_message, $text, "resubmit entry without qualifier \/codon\_start", 
			                 "try without /codon_start" );

   if ( $text =~ /flat file and conceptual translations differ by X/ )
   {
      add_message ( $r_message, $text, "flat file and conceptual translations differ", 
   ...
   add_message ( $r_message, $text, "translational exception not within frame", 
                                         "transl_except not in frame" );
   add_message ( $r_message, $text, "\/exception.*no flat file translation", 
                                         "/exception no translation" );
   add_message ( $r_message, $text, "stop-codon-only CDS must be at 3\' end and partial", 
                                         "must be 3' partial" );
   add_message ( $r_message, $text, "more than one stop codon at 3\' end", 
                                         "contains stop" );
   add_message ( $r_message, $text, "incorrect or missing \/codon\_start qualifier", 
                                         "add or correct /codon_start" );
   add_message ( $r_message, $text, "stop codon at 3\' end - location must not be partial", 
                                         "remove 3' partial" );
   add_message ( $r_message, $text, "differs from standard table",
                                         "/transl_table differs" );

   if ( ! $$r_message ) 
   { 
      $$r_message = ( $type eq "W" ) ? "warning" : "error"; 
   }
}
...
sub add_message 
{
  ...
  my ( $r_message, $text, $pattern, $display ) = @_;
  ...
  if ( $text =~ /$pattern()/ ) 
  {
	...
	$$r_message .= "; ";
     ...
     $$r_message .= ( defined $display ) ? $display.$1 : $pattern.$1;
  }
}
